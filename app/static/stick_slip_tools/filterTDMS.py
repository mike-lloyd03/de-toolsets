...
def filter_and_interpolate_data (path_to_file):
    ...
    # Filter out all rows where the CoF values is equal to that in the following row
    ...
    large_step_arr = (filtered_data.shift(periods=-1, axis=0)['Cycle Count'] - filtered_data['Cycle Count'] > .5)
    filtered_data.loc[large_step_arr, 'Cycle Count'] = filtered_data.loc[large_step_arr, 'Cycle Count'] + .5

    # Remove the cycle count values that are duplicates leaving only the first in the range of duplicates
    duplicate_cycle_values = filtered_data['Cycle Count'] == filtered_data.shift(periods=1, axis=0)['Cycle Count']
    ...
    filtered_data['Cycle Count'] = filtered_data['Cycle Count'].interpolate(method='values', limit_direction='forward')
    ...
    for block in data_blocks:
        data_block = filtered_data[(filtered_data['Cycle Count'] >= min(block)) & (filtered_data['Cycle Count'] <= max(block))]
        if len(data_block):
